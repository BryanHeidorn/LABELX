...
=cut
				...
				my $OCRLine;
				if ( -e $TextFileName )
				{  # if it exits we'll open otherwise keep looking for OCR files
				print "OCR File Name:$TextFileName\n";
					$OCRLine = do {
						local $/ = undef;
				...
				my $XMLFileName2;
				# removed to allow output XML to be anywhere and not just relative to CSV
				#$XMLFileName2 = $RootofCSV . $XMLOutputRootDirectory . $NewXMLFileName2;
				$XMLFileName2 = $XMLOutputRootDirectory . $NewXMLFileName2;
...
# 				Every time we need to search a CSV line we need to restore the $UTF8OCRLine since it is "used up"
   ...
   #stores sorted keys by string length. We'll try to match longer strings first
				...
				for ( $j = 0 ; $j < @SortedDwCContentsContents ; $j++ ) {
					...
					#print $SortedDwCContentsIndices[$j]. "\n"; 
...
=cut
	  ...
	  #print "This item is already found $SortedDwCContentsIndices[$indexE] \n";
				...
				my $FieldList = $xmlparser->load_xml( string => $bracketedXML
    			  	# parser options ...
    				);
...
=cut
						...
						foreach my $Field (  $FieldList->findnodes('/labeldata/ot') ) {
...
#				The illegal XMLparser rountines in the next program cannot accept characters such as "<" in XML contents 
				#$XMLOutLine = RestoreXMLSpecialChar($XMLOutLine);
				#print "Befor cleanup 2:\n$XMLOutLine\n";
				# Removes null tags, merges agacent like items and mor
				$XMLOutLine = CleanXML($XMLOutLine);
				#print "After cleanup 2:\n$XMLOutLine\n";
				#print "XMLOutLine before RestoreSpacing\n$XMLOutLine\n";
				my $OCRLinewXMLChars = XMLSpecialChars($OCRLine);
				$XMLOutLine = RestoreOCRSpacing( $XMLOutLine, $OCRLinewXMLChars );
...
=cut
	...
	while(@ChangeArray) {
		$newChangeLine .= shift(@ChangeArray);	
	} 
